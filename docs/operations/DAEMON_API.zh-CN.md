...
对于不需要常驻 daemon 的编排器，请使用 `nano binary exec`。它接受 prompt 参数；当省略参数时，从 stdin 读取 prompt：
...
Binary 模式默认追加 `<<<NANO_RESULT>>>` JSON 摘要，并支持 `NANO_BINARY_RESULT_FORMAT=plain|json|both`、语义化退出码、`--sandbox=auto|on|off` 和 `--on-exit-cmd`。完整的嵌入契约参见 `docs/integration/EMBED_AS_ENGINE.md`。
...
`/events` 返回 active task 与 team session 存储中的事件。`/audit` 应用审计过滤，覆盖 sandbox、审批、权限决策元数据与错误事件。
...
| WS close 1006 | abnormal close | 使用 since_seq 重连 |
