...
DEPLOYMENT_DIR="${DEPLOYMENT_DIR:-$SCRIPT_DIR}"
...
render_config_locally() {
    local template_path="${DEPLOYMENT_DIR}/daemon-config-template.yaml"
    ...
    if command -v envsubst >/dev/null 2>&1; then
        ...
        if command -v python3 >/dev/null 2>&1; then
            ...
            NANO_CONFIG_TEMPLATE_PATH="$template_path" NANO_CONFIG_RENDERED_PATH="$rendered_path" \
            python3 - <<'PY'
import os, sys
src = os.environ['NANO_CONFIG_TEMPLATE_PATH']
dst = os.environ['NANO_CONFIG_RENDERED_PATH']
...
func main(){
    in := os.Getenv("NANO_CONFIG_TEMPLATE_PATH")
    out := os.Getenv("NANO_CONFIG_RENDERED_PATH")
...
GO
                env NANO_CONFIG_TEMPLATE_PATH="$template_path" NANO_CONFIG_RENDERED_PATH="$rendered_path" go run /tmp/render_config.go >/dev/null 2>&1 || {
...
transfer_files() {
    ...
    scp -o StrictHostKeyChecking=no -i $PEM_FILE "${DEPLOYMENT_DIR}/daemon-monitor.sh" $EC2_USER@$EC2_HOST:$DEPLOY_DIR/
    if [ $? -ne 0 ]; then
