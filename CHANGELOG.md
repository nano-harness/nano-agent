...
- **agent**: Per-file byte budget for instruction files (NANO.md / `.nano/rules`) inlined into the system prompt. New `instruction_file_max_bytes` config (default 32KB, env `NANO_INSTRUCTION_FILE_MAX_BYTES`); oversized files are truncated head/tail with an explicit in-context omission notice instead of silent truncation (industry practice: Claude Code ~32KiB, OpenClaw ~12K chars, Hermes ~25K chars).
- **tools/shell**: Built-in sensitive environment variable blocklist (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `AWS_SECRET_ACCESS_KEY`, etc.) is no longer inherited by spawned shell commands, closing the "child process inherits parent env, `printenv` leaks tokens" residual risk. Listing a key in `allowed_env_vars` exempts it.
- **skill**: Per-session skill activation counts tracked in the skill manager (foundation for `/skill:doctor`).
