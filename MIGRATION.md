# Migration Guide: Single-Agent to Swarm Mode
...
### Scenario 3: You Ran Long-Running Daemon Tasks

**Before**:
```bash
# Execute command and wait
curl -X POST http://localhost:4380/api/v1/sessions/execute \
  -d '{"command": "complex task", "session_id": "sess1"}'
```

**After**:
Create a team session for better management:
```bash
# Create team session
SESSION_ID=$(curl -X POST http://localhost:4380/api/v1/teams/sessions \
  -H "Content-Type: application/json" \
  -d '{"team_name": "worker-team"}' | jq -r '.session_id')

# Execute in team context
curl -X POST http://localhost:4380/api/v1/teams/sessions/$SESSION_ID/execute \
  -H "Content-Type: application/json" \
  -d '{"command": "complex task with parallel subtasks"}'
...
- Mailbox for status updates

### Scenario 4: You Built Custom LLM Integrations

**Before**:
Direct agent creation and execution:
```go
agent, _ := agent.New(cfg, approvalHandler)
agent.Process(ctx, "task")
```

**After**:
Use engine layer for team support:
```go
// For single-agent mode (unchanged)
eng, _ := engine.New(cfg, approvalHandler)
eng.Agent.Process(ctx, "task")

// For team-lead mode
eng, _ := engine.NewLeadEngine(cfg, approvalHandler, "team-name")
eng.Agent.Process(ctx, "task with team coordination")
```

**Benefits**:
- Engine handles mailbox initialization
- Proper tool registration (spawn_teammate)
- Session management
...
Delegate independent subtasks to teammates:

```
Team-Lead: "Build a complete REST API"
  ├─ Teammate 1: Create database models
  ├─ Teammate 2: Implement authentication
  ├─ Teammate 3: Write API handlers
  └─ Team-Lead: Integrate components
```

### 2. Specialized Agents

Create teammates with specific roles:
...
```json
{
  ...
  "parameters": {
    ...
    "task": "audit authentication code for vulnerabilities"
  }
}
```

### 3. Status Updates via Mailbox

Teammates can send progress updates:

```json
{
  "tool": "send_message",
  "parameters": {
    "message": "Found 3 SQL injection vulnerabilities in auth.go",
    "priority": "high"
  }
}
```

### 4. Long-Running Team Sessions

Daemon team sessions persist across requests:

```bash
# Create once
SESSION_ID=$(create_team_session "worker-team")

# Use multiple times
execute_in_team $SESSION_ID "task 1"
execute_in_team $SESSION_ID "task 2"
execute_in_team $SESSION_ID "task 3"

# Cleanup
delete_team_session $SESSION_ID
```
...
- **Documentation**: See [SWARM.md](./SWARM.md) for detailed swarm documentation
