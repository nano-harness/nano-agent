...
func TestLSTool(t *testing.T) {
	...
	tempDir, err := os.MkdirTemp("", "lstool_test")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	...
	os.Mkdir(subDir, 0755) //nolint:errcheck

	// Create test files
	os.WriteFile(filepath.Join(tempDir, "file1.txt"), []byte("content1"), 0644) //nolint:errcheck
	os.WriteFile(filepath.Join(tempDir, "file2.go"), []byte("content2"), 0644)  //nolint:errcheck
	os.WriteFile(filepath.Join(tempDir, ".hidden"), []byte("hidden"), 0644)
	os.WriteFile(filepath.Join(subDir, "nested.txt"), []byte("nested"), 0644)

	tool := NewLSTool(tempDir, nil, nil)
	...
	tests := []struct {
	...
	}{
		{
			...
			validate: func(t *testing.T, result interface{}) {
				...
				if !ok {
					t.Error("Expected result to be []FileInfo")
					return
				}
				if len(files) < 3 { // At least file1.txt, file2.go, subdir
		...
		},
		{
			...
			validate: func(t *testing.T, result interface{}) {
				...
				if !found {
					t.Error("Expected to find .hidden file when show_hidden=true")
				}
			},
		},
		{
			...
			validate: func(t *testing.T, result interface{}) {
				...
				found := false
				for _, file := range files {
					if file.Name == "nested.txt" {
						found = true
						break
					}
				}
				if !found {
					t.Error("Expected to find nested.txt in recursive listing")
				}
			},
		},
		{
