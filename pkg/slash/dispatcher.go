...
// handled. Handled=false means the dispatcher did not recognize the command
// and the caller should fall through to its existing slash-command pipeline
// (backend forwarding, permission handler, etc.).
type Result struct {
	Handled bool
