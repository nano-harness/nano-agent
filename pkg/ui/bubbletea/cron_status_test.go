...
func TestUpdate_CronStatusMsgSetsIndicator(t *testing.T) {
	m := newTestModel(80)
	got, cmd := m.Update(CronStatusMsg{Indicator: "⏰ 2 scheduled, 2 running"})
	if cmd != nil {
		t.Fatal("CronStatusMsg should not return a command")
	}
	updated := got.(*Model)
	if updated.cronIndicator != "⏰ 2 scheduled, 2 running" {
...
func TestRenderInputSectionShowsCronIndicator(t *testing.T) {
	m := newTestModel(80)
	m.cronIndicator = "⏰ 2 scheduled, 2 running"
	m.status = "等待输入"

	var b strings.Builder
	m.renderInputSection(&b)
	out := b.String()
	if !strings.Contains(out, "⏰ 2 scheduled, 2 running") || !strings.Contains(out, " | 等待输入") {
...
func TestRenderInputSectionNarrowWidthTruncatesWithIndicator(t *testing.T) {
	...
	m.cronIndicator = "⏰ 123456789 scheduled, 9 running"
...
func TestUpdate_CronStatusMsgDoesNotAlterPhase(t *testing.T) {
	...
	_, cmd := m.Update(CronStatusMsg{Indicator: "⏰ 1 scheduled, 1 running"})
	if cmd != nil {
